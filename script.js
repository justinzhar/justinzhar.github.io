...
function initScrollAnimations() {
    ...
    // Observe sections (skip skills - show immediately)
    ...
    // Staggered animation for project cards - alternating left/right
    const cardObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting) {
                const card = entry.target;
                const cardIndex = Array.from(document.querySelectorAll('.project-card')).indexOf(card);
    ...
    style.textContent = `
        ...
        .project-card.slide-from-left {
            transform: translateX(-100px) scale(0.95);
        }
        
        .project-card.slide-from-right {
            transform: translateX(100px) scale(0.95);
        }
